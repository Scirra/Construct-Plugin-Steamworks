...
class Steamworks_ExtInstance extends globalThis.ISDKInstanceBase {
    ...
    async #getSteamLeaderboardData(leaderboardName) {
        ...
        if (result["isOk"]) {
            data = {
                ...
                hSteamLeaderboard: result["hSteamLeaderboard"],
                entries: []
            };
    ...
    _getLeaderboardEntryCount(leaderboardName) {
        const data = this.#leaderboardData.get(leaderboardName);
        return data?.entries.length ?? 0;
    }
    _getLeaderboardEntryAt(leaderboardName, index) {
    ...
    async uploadLeaderboardScore(leaderboardName, score, forceUpdate) {
        ...
        try {
            ...
            const result = (await this._sendWrapperExtensionMessageAsync("upload-leaderboard-score", [data.hSteamLeaderboard, score, forceUpdate]));
            if (result["isOk"]) {
                // Read back result values
                const submittedScore = result["score"];
                const didScoreChange = result["didScoreChange"];
                const globalRankNew = result["globalRankNew"];
                const globalRankPrevious = result["globalRankPrevious"];
    ...
    async downloadLeaderboardEntries(leaderboardName, dataType, start, end) {
        ...
        try {
            ...
            const result = (await this._sendWrapperExtensionMessageAsync("download-leaderboard-entries", [data.hSteamLeaderboard, dataType, start, end]));
            if (!result["isOk"])
            ...
            const hSteamLeaderboardEntries = result["hSteamLeaderboardEntries"];
            ...
            for (let i = 0; i < entryCount; ++i) {
                ...
                promises.push((async () => {
                    const result = (await this._sendWrapperExtensionMessageAsync("get-leaderboard-entry", [hSteamLeaderboardEntries, i]));
                    if (!result["isOk"])
                        throw new Error(`invalid leaderboard entry`);
                    const entry = entries[i];
                    entry.personaName = result["personaName"];
                    entry.globalRank = result["globalRank"];
