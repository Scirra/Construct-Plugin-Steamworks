...
class Steamworks_ExtInstance extends globalThis.ISDKInstanceBase
{
	...
	constructor()
	{
		...
		this.#isOverlayEnabled = properties[2] as boolean;

		// Listen for events from wrapper extension.
		this._addWrapperMessageHandlers([
			["on-game-overlay-activated",	e => this.#onGameOverlayActivated(e as JSONObject)],
			["on-dlc-installed",			e => this.#onDlcInstalled(e as JSONObject)],
	...
	async #init()
	{
		...
		if (this.#isAvailable)
		{
			this.#isRunningOnSteamHardware = result["isRunningOnSteamHardware"] as number;
			this.#isRunningUnderProton = result["isRunningUnderProton"] as boolean;
			this.#personaName = result["personaName"] as string;
			this.#accountId = result["accountId"] as number;
			this.#steamId64Bit = result["steamId64Bit"] as string;
			this.#staticAccountKey = result["staticAccountKey"] as string;
			this.#appOwnerAccountId = result["appOwnerAccountId"] as number;
			this.#appOwnerSteamId64Bit = result["appOwnerSteamId64Bit"] as string;
